...
use crate::vss::transcript::TranscriptMixed;
use crate::fft::fft;
...
impl PolyComDealer {
    ...
    pub fn get_transcript(&self, signers: &Vec<bool>, sigs: Vec<bls12381::Signature>, public_keys: &Vec<G1Projective>, th: usize, deg: usize, h: &G1Projective) -> TranscriptMixed {
        ...
        TranscriptMixed::new(self.coms.clone(), reveal_shares, reveal_randomness, agg_sig, ciphertext, chunk_pf, r_bb, enc_rr, share_pf)
    }
...
impl PolyComReceiver {
    ...
    pub fn verify_transcript(&self, t: &TranscriptMixed, sc: &SharingConfiguration, pp: &PublicParameters, pks: &Vec<&PublicKey>, pub_keys: &Vec<G1Projective>) -> bool {
        let n = t.coms().len();
    ...
    pub fn test_vss(){
        ...
        for th in [4, 8] {
            ...
            for recv in recvs.iter() {
                assert!(recv.verify_transcript(&t, &sc, &pp, &ver_keys, &enc_keys));
            }
